...
public class CProxy
{
    //Este método no regresa nada y va a recibir como parámetro un entero q se llama pOpcion.
    public interface ISujeto
    {
        void Peticion(int pOpcion);
    }

    //La clase ProxySencillo es con la que vamos a estar trabajando, la que va a hacer de intermedieron entre el cliente
    //y la clase CCocina que es el sujeto con el que queremos interactuar, pero solo lo podremos hacer a través del Proxy.
    public class ProxySencillo : ISujeto
    {
        ...
        {
            //Si cocina es = null, es decir si nuestra variable de rederencia de cocina no ha sido todavia, simplemente
            ...
            //Si la pOpcion es = 1, invocamos el método RecetaSecre() de CCocina, si la pOpcion es = 2, invocamos el
            //método de Cocinar() de CCocina colocando un 5 de referencia de que estamos cocinando 5 platos.
            if (pOpcion == 1)
    ...
    private class CCocina
    {
        ...
        {
            Console.WriteLine("Cocinando {0} platos", n);
        }
    }
}